...
<template>
  <Transition name="toast-fade">
    <div
      ...
      :class="
        'error' in syncStore.toast || hasErrors
          ? 'border border-danger text-danger bg-body'
          : 'bg-body border text-muted'
      "
    >
      <template v-if="'result' in syncStore.toast">
        ...
        <div v-for="err in syncStore.toast.result.errors" :key="err" class="mt-1">⚠ {{ err }}</div>
      </template>
      <template v-else>
